...
| **§91 — 7-Ago-2026** | **Fix falsa caída ORG (−27K) y Total (−26K) en MTD**. Root cause 2 capas: (1) ORG (único canal `_canales_cum_desde_nr`) arrastra el acumulado en días sin fila real → `nr_data_maxday` con `cum>0` lo contaba como día con dato → comparaba Ago 1-5 vs Jul 1-6. Fix: `_nr_last_real_day` = último día con fila real (`len(d_df)>0`). (2) Propagación agregados `max`→`min` (día hasta el que TODOS los hijos tienen dato). Validado offline + en vivo: ORG −27,426→+129, Total −26,710→+14,176, UCR Gest −11 (=ref Andrés). Solo `src/processors.py`. Tag `v-pre-org-maxday-fix`. Apps Script v27. |
| **§92 — 11-Sep-2026** | **Migración SSOT de Installs + alarma de frescura.** `SBOX_MKTCORPMP.BASE_INSTALLS_LIFECYCLE` se congeló el 10-Ago-26 (`created` y `last_modified` a 1 ms → build manual único, sin pipeline). Síntomas: KPI cards en cero, falso −67% MoM en los 9 canales, CPI ~3x. Fuente nueva: **`WHOWNER.LK_MP_INDIVIDUALS_INSTALLS_LIFECYCLE`** (vista certificada, Dataflow, contrato idéntico). Validada antes de migrar: −0.00% vs histórico Ene-25→May-26; 0 installs perdidos en los 8 canales MLM. Solo cambia el `FROM`. Resultado: Sep-26 presente, Ago-26 completo (2,163,592, MoM −2.8%). +**`check_installs_freshness()`** como Paso 0b: alarma **por distorsión de CPI**, no por días. Compara installs vs `managed_max` (corte real de la inversión) — si ambos van atrasados parejo el CPI está sano y no alarma. Umbrales +5% / +15% de inflación implícita, tope absoluto 10 días (congelamiento) y piso de 5 días (arranque de mes no evaluable). Sin auto-ajuste posible — la acción es escalar a Corp. Apps Script **v58**. |
| **§90 — 6/7-Ago-2026** | **Alineación UCR Gest E&G + fixes de lag de fuentes**. (1) UCR Gest → `NR_ADJUST_FCST` (mes en curso) / `NEW+REC_7D_ADJUST` (cerrados) desde `BT_OC_DASHBOARD_ALL_CAMPAIGNS_NR`, `source_tc="campaigns_ucr"` — metodología E&G (Andrés Garza). VPU/costos siguen Torre Daily. (2) Fix comparación LMTD: `nr_data_maxday` (último día real por canal) capa el LMTD al mismo # de días que el MTD cuando campaigns tiene lag → elimina caída artificial −20% UCR. (3) **Fix dump ORG**: constante `_INAPP_MANAGED_CAP = LEAST(MAX Torre Daily, MAX Individuals)` en 3 CTEs INAPP. INAPP carga ~2 días antes que canales gestionados → sin tope, el día más nuevo vuelca todo el sitio a ORG (+57K artificio, Total 285K→200K). (4) **Alarma** `check_source_lag()` en Paso 0: WARNING visible si INAPP va adelante. Tags rollback: `v-pre-ucr-fcst` / `v-pre-lmtd-maxday-fix` / `v-pre-org-residual-cap`. Versión Apps Script 25. |
